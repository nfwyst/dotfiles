...
export class SLTPCalculator {
  ...
  calculate(
    ...
    try {
      ...
      // 计算 TP (基于风险回报比)
      const tp1Distance = stopDistance * 1.0;   // 1:1
      const tp2Distance = stopDistance * 1.5;   // 1:1.5
      const tp3Distance = stopDistance * 2.0;   // 1:2
      ...
      reasoning.push(`TP1 1:1: ${takeProfits.tp1.toFixed(2)} (${side === 'long' ? '+' : '-'}${(tp1Distance / entryPrice * 100).toFixed(2)}%)`);
      reasoning.push(`TP2 1:1.5: ${takeProfits.tp2.toFixed(2)} (${side === 'long' ? '+' : '-'}${(tp2Distance / entryPrice * 100).toFixed(2)}%)`);
      reasoning.push(`TP3 1:2: ${takeProfits.tp3.toFixed(2)} (${side === 'long' ? '+' : '-'}${(tp3Distance / entryPrice * 100).toFixed(2)}%)`);
  ...
  calculateWithATR(
    ...
    if (side === 'long') {
      stopLoss = entryPrice - stopDistance;
      takeProfits = {
        tp1: entryPrice + stopDistance * 1.0,
        tp2: entryPrice + stopDistance * 1.5,
        tp3: entryPrice + stopDistance * 2.0,
      };
    } else {
      stopLoss = entryPrice + stopDistance;
      takeProfits = {
    ...
    return {
      ...
      stopLoss,
      takeProfits,
      riskRewardRatios: { tp1: 1.0, tp2: 1.5, tp3: 2.0 },
      stopDistance,
      timeframe: tf,
