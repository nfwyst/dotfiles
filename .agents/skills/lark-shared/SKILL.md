...
- **优先使用 User 身份**：只要目标是访问或创建用户自己的文档、表格、日历、云空间资源，默认优先用 `--as user`。仅当用户明确要求以应用名义操作，或当前能力只能通过应用身份完成时，才回退到 `--as bot`。
...
2. 提议执行更新（CLI 和 Skills 需要同时更新）：
   ```bash
   bun add -g @larksuite/cli@latest && bunx skills add larksuite/cli -g -y
   ```
