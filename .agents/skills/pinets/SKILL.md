...
```bash
# Global install
bun install -g pinets-cli

# Or run directly with bunx (no install needed)
bunx pinets-cli run indicator.pine --symbol BTCUSDT -q
```
...
When using `bunx`, replace `pinets` with `bunx pinets-cli` in all examples below.
...
- The tool bundles PineTS internally — no additional bun packages are needed at runtime.
