...
Use `bunx` to run mcporter immediately without installing globally:
...
bunx mcporter call tradingview-m.coin_analysis symbol=BTCUSDT exchange=BINANCE timeframe=15m
```
...
For Bun projects, add mcporter as a dev dependency:

```bash
bun add -D mcporter
```

Then use via `bun mcporter`.
...
```json
{
  "mcpServers": {
    "tradingview-m": {
      ...
      "command": "bunx",
      "args": ["-y", "tradingview-mcp-server@latest"]
    },
    "alpha-vantage": {
...
```bash
# Try with bunx (no installation needed)
bunx mcporter list
...
```bash
# Use bunx to bypass any stdio transport issues
bunx mcporter call tradingview-m.coin_analysis symbol=BTCUSDT exchange=BINANCE timeframe=15m

# Test with ad-hoc HTTP URL (for server you control)
bunx mcporter call --http-url "https://api.example.com/mcp" list
...
- Ensure key is exported properly: `export ALPHA_VANTAGE_API_KEY="key_here"`
...
- Try using bunx which has better error handling: `bunx mcporter call ...`
