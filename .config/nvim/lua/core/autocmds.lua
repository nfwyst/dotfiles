...
SET_AUTOCMDS({
	...
	},
	{
		"FileType",
		{
			...
			callback = function(event)
				...
				if TABLE_CONTAINS({ "DressingSelect", "DressingInput" }, event.match) then
					return
				end
	...
	},
	{
		"FileType",
		{
			...
			callback = function()
				SET_OPTS({ wrap = true, tabstop = 2, softtabstop = 2, shiftwidth = 2 }, true)
			end,
...
SET_AUTOCMDS({
	...
	},
	{
		"InsertLeave",
		{
			...
			group = group,
			callback = function()
				SET_OPTS({ cursorline = not TABLE_CONTAINS(INVALID_FILETYPE, vim.bo.filetype) }, true)
			end,
		},
	},
})
