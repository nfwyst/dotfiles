...
require("lint").linters.eslint_d.args = vim.list_extend(require("lint").linters.eslint_d.args or {}, {
  -- Legacy .eslintrc.* cascades up the directory tree, merging every parent
  -- config on top of --config. When a git worktree is nested inside its main
  -- repo, two identical .eslintrc.cjs get merged and `plugin:prettier/recommended`
  -- is declared twice -> "Plugin prettier was conflicted" (eslint_d then emits
  -- plain-text error, surfaced as "Could not parse linter output").
  -- --no-eslintrc disables the cascade so only the resolved --config loads.
  -- Flat config (eslint.config.*) does not cascade and rejects the flag, so we
  -- emit it only for legacy configs; "" is a no-op arg eslint tolerates.
  function()
    local cfg = util.get_file_path(constant.ESLINT, { for_eslint = true, ensure_package = true })
    return cfg and cfg:match("%.eslintrc") and "--no-eslintrc" or ""
  end,
  "--config",
  function()
