...
vim.api.nvim_create_autocmd("PackChanged", {
  ...
  callback = function(ev)
    ...
    -- IMPORTANT: do NOT call blink.cmp's cmp.build() here. That uses a
    -- module-level `native` singleton whose git_commit is snapshotted at the
    -- first require("blink.cmp") on startup -- i.e. the OLD commit, before
    -- PlugSync's offline checkout. The lib would then be named after the old
    -- hash and fail to load on next startup (Rust fuzzy matcher not available).
    -- Instead, create a FRESH managed instance now (post-checkout), so it reads
    -- the new HEAD and names the artifact with the correct commit hash.
    if name == "blink.cmp" and (kind == "install" or kind == "update") then
      ...
      vim.schedule(function()
        ...
        if not vim.tbl_contains(vim.api.nvim_list_runtime_paths(), repo) then
          vim.opt.runtimepath:append(repo)
        end
        local ok_m, managed = pcall(require, "blink.lib.native.managed")
        if not ok_m then
          sync_notify("blink.lib.native.managed require failed: " .. tostring(managed), vim.log.levels.ERROR)
          return
        end
        local logger = require("blink.cmp.logger")
        local fresh = managed.new({
          module_name = "blink.cmp",
          library_name = "blink_cmp_fuzzy",
          current_file_path = repo .. "/lua/blink/cmp/init.lua",
          logger = logger,
        })
