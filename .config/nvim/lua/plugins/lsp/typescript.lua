...
local baseDefinition = hd[lsp_methods.textDocument_definition]
local baseHint = hd[lsp_methods.textDocument_inlayHint]
local basseFold = hd[lsp_methods.textDocument_foldingRange]
...
local function definition(err, result, method, ...)
  if vim.islist(result) and #result > 1 then
    local filtered_result = FILTER_TABLE(result, filterReactDTS)
    return baseDefinition(err, filtered_result, method, ...)
  end

  return baseDefinition(err, result, method, ...)
end

local function foldingRange(err, result, ctx, config)
  ...
  return basseFold(err, result, ctx, config)
end
