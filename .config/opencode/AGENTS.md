...
- 优先级(高 → 低):用户当前消息 > 项目内 `AGENTS.md` > 本全局配置 > agent 默认行为。
...
3. 回显安装命令,等用户确认后执行 `bunx skills add <owner/repo@skill> -y`。
4. 安装完成立即加载。
...
- 包管理优先级:`bun` > `npm`,`bunx` > `npx`,`uv` > `pip`,前者不可用时自动降级到后者。
...
- TS/JS/前端改动后,仓库有配置时执行相关 ESLint 与 TypeScript 类型检查,无配置时跳过。
...
- 删除非本轮生成的文件、批量重构、修改依赖。
- `git commit` / `git push` / `--force` / `--force-with-lease` / 改写历史 / 创建/关闭 PR。
