...
- 禁止以完全相同参数重复调用 `read` 读取同一文件。
- 已读取过某文件后，如果仍需更多信息，必须先说明当前已知结论，再选择以下方式之一继续:
  1. 使用 `grep` / `rg` 搜索具体符号、关键词或调用点。
  2. 使用不同的 `offset` / `limit` 读取新片段。
  3. 基于已有上下文推进，无法推进时向用户说明缺失信息。
- 连续 2 次工具结果没有新增信息时，必须停止继续读取，并输出当前结论、阻塞点和下一步建议。
